...
namespace UD_Granit.Helpers
{
    public static class RightsManager
    {
        public static class Account
        {
            ...
            public static bool Edit(User user, User editedUser) { return ((user.Id == editedUser.Id) || (user is Administrator) || (((user is Member) && ((user as Member).Position == MemberPosition.Chairman) && !(editedUser is Administrator)))); }

            public static bool Remove(User user) { return (user is Administrator); }
            public static bool Remove(User user, User removedUser) { return ((user is Administrator) || (user.Id == removedUser.Id) || ((user is Member) && ((user as Member).Position == MemberPosition.Chairman))); }

            public static bool ShowPhone(User user) { return ((user is Member) || (user is Administrator)); }
            public static bool ShowPhone(User user, User showedUser) { return ((user.Id == showedUser.Id) || (user is Member) || (user is Administrator)); }
        }
